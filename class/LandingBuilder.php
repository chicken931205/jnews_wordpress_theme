...
class LandingBuilder {
	...
	public function get_include_post() {
		if ( vp_metabox( 'jnews_page_loop.post_sticky', false, $this->post_id ) ) {
			$sticky_post = get_option( 'sticky_posts' );
			if ( ! empty( $sticky_post ) ) {
				$sticky_post = implode( ',', $sticky_post );
				return empty( vp_metabox( 'jnews_page_loop.include_post', null, $this->post_id ) ) ? $sticky_post : $sticky_post . ',' . vp_metabox( 'jnews_page_loop.include_post', null, $this->post_id );
			}
		}

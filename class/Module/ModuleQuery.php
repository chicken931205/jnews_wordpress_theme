...
Class ModuleQuery {
	...
	private static function default_query( $attr ) {
		$include_category = $exclude_category = $result = $args = $inc_args = array();
		$found_post       = 0;
		$included_only    = false;
		...
		$needed_post                 = $args['posts_per_page'];
		$post_offset                 = $args['offset'];

		/* see SDan7CUJ */
		if ( ! empty( $attr['include_post'] ) ) {
			$args['orderby'] = 'post__in';
			if ( isset( $attr['included_only'] ) && in_array( $attr['included_only'], array( 'yes', 'true' ) ) ) {
				$args['post__in'] = array_unique( explode( ',', $attr['include_post'] ) );
				$included_only    = true;
			} else {
				$inc_args             = $args;
				...
				if ( self::stil_has_post( count( $inc_args['post__in'] ), $args['paged'], $args['offset'] ) ) {
					...
					$found_post     = $found_post + count( $inc_args['post__in'] );
				}
			}
		}
		...
		// order
		if ( isset( $attr['sort_by'] ) && ! $included_only ) {
		...
		if ( ! empty( $attr['include_post'] ) && ! $included_only && $included_posts ) {
			...
			if ( count( $included_posts->posts ) < $needed_post ) {

				foreach ( $query->posts as $post ) {
					$result[] = $post;
				}
			}
			$found_post = $found_post + $included_posts->found_posts + $query->found_posts;
		} else {
			foreach ( $query->posts as $post ) {
				$result[] = $post;
			}
			$found_post = $found_post + $query->found_posts;
		}
		...
		return array(
			'result'     => $result,
			'next'       => self::has_next_page( $found_post, $args['paged'], $post_offset, $attr['number_post'], $attr['pagination_number_post'] ),
			'prev'       => self::has_prev_page( $args['paged'] ),
			'total_page' => self::count_total_page( $found_post, $args['paged'], $post_offset, $attr['number_post'], $attr['pagination_number_post'] ),
		);
	}
